...
- [ ] **D.5.27 — Remote Mode (per ADR-0032).**
  - **V1 (no new ingests):** "Remote" masthead pill toggles the mode. Activating swaps the metric switcher to a purchasing-power-focused set, opens a Remote Jobs panel (reuses `JobList.svelte`, geography chips hidden, all other filters honored), and reveals a Locality Leaderboard (sortable table: locality, posting count, RPP, GS-13 step 1, purchasing-power index, FRPP count). Pay anchor is editable inline and defaults to the user's last Compensation Comparator selection. Choropleth renders at county granularity using `cost_of_living_index.by_county` + `pay_scales` 2026; falls back to state when county COL is missing and labels the precision honestly. Anywhere-remote vs duty-station-locked badging on every JobCard and JobList row. New `public_map/src/lib/remote.ts` wraps `compensation.compute()` — no duplicate calculator path. Tests cover the purchasing-power formula, leaderboard sort stability, anchor change re-render, and the duty-station-locked badge logic.
  - **V1.1 (three new ingests, each independent):**
    - State tax burden: `scripts/ingest_state_tax_burden.py` from Tax Foundation public CSV; checked-in seed; new `state_tax_burden` table indexed by state code.
    - Broadband availability: `scripts/ingest_fcc_broadband.py` from FCC National Broadband Map county-level fixed ≥ 100/20 Mbps; checked-in seed; new `county_broadband` table.
    - Airport distance: `scripts/ingest_faa_airports.py` from FAA Part 139 list; checked-in seed; new `airports` table; distance computed on export.
    - Each adds one column to the leaderboard, one tooltip on the choropleth, one row in `data_source_status`, and self-bootstraps per ADR-0027.
  - **Out of scope:** subjective composite quality-of-life scores (Sperling's, AreaVibes, Niche), school ratings, crime stats, climate normals, walkability scores, schools-by-attendance-zone. Per ADR-0032 §6.

  **Exit:** Remote Mode toggle works; choropleth shades counties by purchasing-power for the active anchor; leaderboard sorts on every column without losing selection; remote-only JobList honors all non-geographic filters; anywhere-remote vs duty-station-locked badge appears on every job; duty-station-re-designation note shows on anywhere-remote jobs; `npm test` and `npm run check` are green; manifest emits a `remote_mode_ready: true` flag once V1 ships and `remote_mode_v1_1: true` once the three new sources are populated.
