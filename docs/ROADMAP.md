...
- [~] **D.5.28 — Browse-first mosaic redesign (per ADR-0033).** *(Mocks shipped 2026-05-11, then re-cut to **rev 2** on 2026-05-12 — see `public_map/mocks/browse/`. Rev 2 is the current target: one filter bar (no separate metric switcher), metrics folded into the area card, "Save as Job List" / "Save to My Postings" verbs with a provenance toast. The mobile dock is **four tabs — Map / List / Here / Saved**; rev-1's separate "Metrics" tab is gone and "Area" is renamed "Here".)*
  - **Status (2026-05-16).** PRs #22 and #24 shipped the `/browse` route — the mobile dock shell plus a working List tab (agency picker, More-filters disclosure, sorted/paginated list, Save/Hide). Map / Here / Saved are honest stubs. The desktop mosaic is **not** built; it exists only as the `desktop-mosaic.html` mock.
  - **Cleanup — shipped code diverged from this plan.** The List tab was built as a fresh inline list inside `public_map/src/routes/browse/+page.svelte` with its own `filterJobDetails` path instead of reusing `JobList.svelte`. **Decision: `JobList.svelte` is the single list component.** Rework `browse/+page.svelte` to render `JobList.svelte` in `richMode` and delete the parallel list; reconcile `AgencyPicker.svelte` with `FilterPanel`'s existing agency picker so there is one agency-picker implementation. D.5.28 stays `[~]` until this lands.
