...
- [~] **D.5.28 — Browse-first mosaic redesign (per ADR-0033).** *(Mocks shipped 2026-05-11, then re-cut to **rev 2** on 2026-05-12 — see `public_map/mocks/browse/`. Rev 2 is the current target: one filter bar (no separate metric switcher), metrics folded into the area card, "Save as Job List" / "Save to My Postings" verbs with a provenance toast. The mobile dock is **four tabs — Map / List / Here / Saved**; rev-1's separate "Metrics" tab is gone and "Area" is renamed "Here".)*
  ...
  - [x] **Pulse band inside `SmallestAreaCard` (placeholder shipped 2026-06-11).** Four headline cells — Open postings, New in last 7d, Median window, Closing ≤ 3d — read `mapState.areaPulse`; while it is null they render dashed-border `data-status="placeholder"` cells with em-dashes and the "PLACEHOLDER — needs historical slice" caption, never fabricated numbers. Deltas render per-cell from `areaPulse.deltas` when a slice lands. `AreaPulse` shape is defined in `store.svelte.ts` (everything nullable).
