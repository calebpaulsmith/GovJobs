...
- [ ] D.5.21 — Per-job historical postings window. When viewing a job card, a "History" tab shows all prior announcements that share the same `position_id` (exact match) plus a second tier of "similar postings" matched on agency + series + grade range + state. Data comes from the local HistoricJoa DB via a new `src/public_map_export.py::job_history_payload` endpoint, writing `job_history.json` (keyed by `position_id`). The tab shows a small chronological list of prior announcements (open date, close date, salary range, location) and a one-line trend label ("Posted 3× in the last 2 years"). No LLM; matching is pure SQL. The exporter only writes history entries for position IDs that appear in `jobs.geojson` to keep the file small.
- [ ] D.5.22 — Posting timeline chart (filter-scoped). A sparkline/bar-chart panel showing number of postings opened per month over the trailing 24 months, scoped to the currently active filter set (agency, series, grade, location, keyword). Data comes from a new `timeline.json` export written by `src/public_map_export.py::timeline_payload`: one row per `(year, month)` for all HistoricJoa rows in the DB, plus per-filter-key pre-computed slices for the top-N agency and series values so the client can recompute the filtered view without a server round-trip. The chart renders in a collapsible panel (bottom-right, above MetricSwitcher, or as a tab in the FeaturePanel) using plain SVG — no chart library. The x-axis is months, the y-axis is posting count, the bars are colored by the active metric. Clicking a bar zooms the time window. The panel title reads "Postings over time — [filter summary]".
- [ ] D.5.23 — Quick-add filter chips. Every displayed job attribute that maps to a filterable field shows a small "+" button on hover/focus. Clicking it adds that value as a filter chip without navigating away. Targets: agency code (→ agency chip), series (→ series chip), grade (→ grade min/max), state (→ geo scope chip), locality code (→ locality scope chip), remote status (→ remote toggle), pay plan (→ pay plan chip). Implemented as a `QuickAdd.svelte` micro-component that wraps any value string and accepts a callback; wired into `JobCard.svelte`, `JobList.svelte` row cells, `StateRoundup.svelte`, and `LocalityDetail.svelte`. Must not duplicate existing chip logic — calls the same `updateFilters` path already used by `FilterPanel.svelte`.
