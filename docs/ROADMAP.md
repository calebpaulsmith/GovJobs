...
- [ ] D.5.21 — Per-job historical postings window: a "History" tab on every JobCard. Backend queries `jobs` by `position_id` (exact prior announcements) plus a second tier of "similar" matches on agency + series + grade. Exporter writes `job_history.json` keyed by `position_id`. Frontend renders a compact chronological list with a trend label (e.g. "Posted 3× in the last 2 years"). Pure SQL — no LLM.
- [ ] D.5.22 — Posting timeline chart: a collapsible SVG sparkline (no chart library) rendered above the MetricSwitcher showing monthly posting counts for the active filter set. Backend exports `timeline.json` with precomputed totals plus slices by agency / series / state so the client can approximate filtered views without a server. Labeled "approximate" because the data is a static snapshot.
- [ ] D.5.23 — Quick-add filter chips: a `QuickAdd.svelte` micro-component that wraps any displayed value with a `+` button on hover. Wired into `JobCard`, `JobList` rows, `StateRoundup`, and `LocalityDetail`. Must call the same `updateFilters` path `FilterPanel` uses — no second write path.
