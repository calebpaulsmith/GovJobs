...
- [ ] **D.5.28 — Browse-first mosaic redesign (per ADR-0033).**
  ...
  - [ ] `layout.ts` accepts `mode: 'browse' | 'map-only'`; per-mode slot rectangles for desktop (≥ 1024 px), tablet (640 – 1023 px), mobile (< 640 px). Tests assert non-overlap for both modes at all three breakpoints.
  - [ ] Browse layout slots — desktop: map (top-left 50%×50%), `SmallestAreaCard` (top-right 50%×50%), `JobList` rich-mode (bottom 100%×50%). Tablet: map + smallest-area stacked top (33% h), list bottom (67% h). Mobile: map thumbnail (200 px) → smallest-area chip → list, with tap-to-expand modal map.
  ...
  - [ ] `mapState` adds `viewMode`, `priorViewport`, `selectedJobId`, `listScrollOffset`.
  ...
  - [ ] Tests: per-mode layout non-overlap (3 breakpoints × 2 modes); click-to-zoom round trip restores `priorViewport`; scroll-away threshold; smallest-area tie-break formula; rich-mode row rendering for the four common cases (single-coord, multi-location, anywhere-remote, no-coords).
