...
- [x] D.5.7 — Corpus growth: Data Admin presets for federal-wide and top-25 imports + trailing-90-days HistoricJoa overlay; closed postings rendered as faint gray dots. *(Infra shipped 2026-05-08: `src/public_map_corpus.py` adds recon-gated Data Admin presets for federal-wide current, top-25 current, and HistoricJoa trailing-90-day closed imports; exporter writes `closed_jobs.geojson`; UI renders a toggleable gray closed-postings layer. **Open-postings floor met — verified against the live `map.thegrandpipeline.com/data/manifest.json` bundle (generated 2026-06-09, reference_year 2026): `job_count` = 12,524 distinct open USAJOBS postings, comfortably past the ≥5,000 floor in invariant #13. Note `feature_count` = 75,064 is the duty-location marker count (`jobs.geojson`), not distinct postings — a multi-location posting yields several features; do not conflate the two (CLAUDE.md hard rule 2 precision). Caveats on the live bundle: `closed_jobs.geojson` = 0 (the trailing-90-day overlay populates only after a HistoricJoa trailing-90 import runs), and `localities.geojson` = 38 of the 58 OPM areas.**)*
...
- [x] D.5.14 — 2026 GS pay-table cutover: ingest official 2026 GS base and locality tables, make 2026 the exported reference year, and verify sampled rows in Public Map Admin before V1 deploy. *(**Verified complete 2026-06-10.** The seed the ingest reads is `data/external/opm_gs_pay/2026.official.csv` (8,850 rows = 150 base + 58 locality areas × 150), sourced from OPM's published `salary-tables/xml/2026/GS.xml` — the **official** 2026 table, not a computed placeholder. Verified cell-by-cell against opm.gov Salary Table 2026-GS and 2026-DCB ("Incorporating the 1% General Schedule Increase, Effective January 2026"): all 150 base cells matched with zero mismatches, and the DCB locality spot-check (GS-13 step 1 = $121,785 at the official 33.94% adjustment) matched too. Note the 2026 base is **lower** than 2025 (GS-1/1 22,584 < 22,886) because OPM restructured the base/locality split — the naive `2025 × 1.01` computation the old orphan `2026_base.csv` used was wrong by hundreds-to-thousands of dollars per cell; that orphan (never read by any code) was deleted. `tests/test_ingest_scripts.py::test_gs_pay_seed_is_official_opm_2026_not_placeholder` codifies the four checkpoints so a future re-compute regresses loudly. `pages/11_Public_Map_Admin.py`'s "Reference year (D.5.14)" panel now auto-compares the operator's `pay_scales` against the known-official values and shows a green ✓ when they match — no manual PDF lookup needed. `current_reference_year(conn)` resolves to 2026. **Confirmed on the live deployment 2026-06-10:** `map.thegrandpipeline.com/data/manifest.json` reports `reference_year: 2026` and `opm_gs_pay` row_count 8,850, and `data/pay_tables.json` carries the official base values (GS-1/1 = 22,584, GS-13/1 = 90,925, GS-15/10 = 164,301) plus the DCB locality cell (GS-13/1 = 121,785) — not the old placeholders.)*
...
- [x] D.5.19 — Local profile job state: viewed badges, Save Job, Hide Job, default exclusion of hidden jobs from map/list/search views, and profile drawers for Saved Jobs, Hidden Jobs, and viewed jobs that have closed. *(Shipped 2026-05-09: `jobProfile.svelte.ts` singleton persists viewed/saved/hidden in `localStorage` under `fedfinder.public_map.job_profile.v1`; `ProfileDrawer.svelte` is a right-side drawer with Saved/Hidden/Viewed-Closed tabs; `mapState.hiddenJobIds` + `excludeHidden()` filter hidden jobs from job sources and heat on every filter cycle; JobCard marks viewed on mount, has Save/Hide toggle buttons; "My Jobs" pill in masthead opens the drawer with a saved-count badge.)*
...
- [x] **D.5.31 — Browse finds jobs; Analysis analyzes places (per ADR-0039, 2026-09-24).** The Browse "Here" tab moved to the Localities screen, renamed **Analysis** (`/analysis`; `/localities` redirects). Analysis picks one area at a user-chosen level (Nationwide · State · Locality · Metro · County), defaulting to where the user was in Browse; `AreaAnalysis.svelte` (ex-SmallestAreaCard) scopes by the chosen area + the user's non-geographic filters, with county/metro via client-side point-in-polygon (`geo.ts::pointInGeometry`, `analysisArea.ts`). Browse's mobile sheet is Postings-only with a job-detail view; the desktop top-right pane is the job pane; polygon taps narrow the list (county/CBSA list scopes fixed — they matched nothing before) and offer "Analyze this area →". The locality rollup is kept as a section on Analysis. Tests: `analysisArea.test.ts`, `tests/analysis-webkit.spec.mjs` + updated WebKit specs.
- [~] **D.5.27 — Localities screen (per ADR-0032).**
  - **Status (2026-06-26).** V1 SHIPPED (PR #82): `/localities` route + enabled masthead pill on `/browse`; `LocalityRollup.svelte` centerpiece (sortable Locality / Postings (default desc) / Pay range / Pay-plan mix + GS-coverage / Cost-of-living-RPP columns, each with a D.5.13 InfoTooltip); multi-select + "Show N jobs in M localities" drill-in and single-row shortcut, both encoding `locality:<code>` chips into the `/browse` URL (preserves non-geographic filters, survives Browse's URL hydration, and is shareable); one-click Remote-only preset; JobList anywhere-remote redesignation note (ADR §9). Rollup math lives in `public_map/src/lib/localities.ts` and aggregates over **jobs.geojson features** (which carry `locality_code`; `jobs_detail.json` does not), deduping postings per locality and dropping geography/radius chips. `manifest.localities_screen_v1: true`.
  - **Both fast-follows SHIPPED (2026-06-26).** (1) The **opt-in GS-anchored purchasing-power column** (ADR §6): a "GS purchasing power" toggle adds a sortable "GS-13 real pay" column = `gs13_step1_locality ÷ (RPP ÷ 100)` (GS-13 pay in U.S.-average-cost dollars), each cell labeled with the locality's GS-coverage % and a formula/source tooltip; off by default since it's GS-only. (2) The **paired highlight map** (ADR §5): `LocalityMiniMap.svelte` shows the locality polygons; selection is two-way — `selected` is lifted to the route and shared, so checking rows highlights polygons and clicking polygons checks rows (no choropleth; color is selection-only). Tests now: `localities.test.ts` (14) + `tests/localities-webkit.spec.mjs` (15, real WebKit, incl. GS column toggle + map render).
  - **FRPP-count column** dropped from V1 — `federal_properties` has no locality join (point-in-polygon needed); revisit with V1.1.
  ...
  - **V1.1 (three new ingests, each independent):** *(State tax SHIPPED 2026-06-26; broadband + airport remaining → flip `localities_screen_v1_1` once all three are populated.)*
    - **State tax burden — SHIPPED.** `scripts/ingest_state_tax_burden.py` (self-bootstrapping per ADR-0027), Tax Foundation "State and Local Tax Burdens" effective rate, checked-in seed `data/external/state_tax_burden/2022.csv` (50 states + DC, CY2022, fetched from the source), new `state_tax_burden` table (schema v15), orchestrator step, `state_tax_burden()` exporter → `state_tax.json`. Rollup gains a sortable "State tax" column (state-level, shown via each locality's primary state and labeled approximate for multi-state localities; hidden when the bundle has no `state_tax.json`). Tests: ingest (seed accuracy + upsert), exporter payload, `localities.ts` resolution + sort, WebKit column render.
...
- [~] **D.5.28 — Browse-first mosaic redesign (per ADR-0033).** *(Mocks shipped 2026-05-11, then re-cut to **rev 2** on 2026-05-12 — see `public_map/mocks/browse/`. Rev 2 is the current target: one filter bar (no separate metric switcher), metrics folded into the area card, "Save as Job List" / "Save to My Postings" verbs with a provenance toast. The mobile dock is **four tabs — Map / List / Here / Saved**; rev-1's separate "Metrics" tab is gone and "Area" is renamed "Here".)*
  - **Status (2026-06-11).** Shipped: the `/browse` route + mobile dock shell + List tab (PRs #22/#24), `locality_code` export + locality-filter fix (#28), the JobList unification (#29), the Here tab `SmallestAreaCard` (#32), the Saved tab (#33), the JobList rich-mode sticky toolbar + facets (#34), the live Map embedded in the Map tab (#35), Browse-mode polygon-click → `geo:` chip (#36/#37), summary/qualifications excerpts in rich rows (≤ 200 chars, HTML-stripped at export in `src/public_map_export.py::_excerpt` → `jobs_detail.json`; 6,419 populated in the latest bundle; landed via a direct-to-master commit, not a numbered PR), the `AgencyPicker` ↔ `FilterPanel` reconciliation (both surfaces share `FilterFields`; orphan deleted 2026-06-11), and **the desktop mosaic (2026-06-11, WebKit-verified)** — see the layout-slots checkbox below. The same day's follow-on session shipped the **row↔marker hover crossfilter**, the **sticky-toolbar URL round-trip** (toolbar now also on the main Postings pane; saved searches v3), and the **pulse-band / annotation placeholder slots plus the per-job historical-badge data feed** — see the checkboxes below. All data-slice questions are now resolved (see the slice checkboxes below): `area_pulse` went live 2026-06-12 as client-side compute, the volume sparkline shipped 2026-06-12 as a click-to-load consumer of the existing `/api/job-history` Function (`AreaTrendSparkline.svelte` on the Here card — no `posting_volume_history.json` file), and the "What to watch" note shipped 2026-06-12 as deterministic + keyless client-side compute over the same Function (`AreaWatchNote.svelte`, per ADR-0036 — the external-model area-summary Function is declined). Still open: click-to-zoom (parked per the 2026-06-11 card-only decision) and the masthead `Localities` pill (blocked on D.5.27). D.5.28 closes via the D.5.12 verification pass.
  - **Cleanup — resolved.** The List tab no longer carries a parallel inline list: `JobList.svelte` is the single list component, used in `richMode` by `/browse` and scoped mode by `/map` (shipped #29). The agency-picker reconciliation is done too (verified 2026-06-11): both surfaces render the shared `FilterFields.svelte` (`/map` via `FilterPanel`, `/browse` via `FilterSheet`), whose agency input is a `MultiSelect` fed by `loadAgencyOptions()`. The orphaned `AgencyPicker.svelte` (zero imports) was deleted.
  - **~~Known bug~~ — resolved (verified 2026-06-11).** `locality:` chips now match in `matchesJobDetail` / `jobDetailGeographyMatches`: the exporter writes each posting's first-duty-station `locality_code` into `jobs_detail.json` (shipped #28) and `filters.ts` matches on it, with cases in `filters.test.ts`. `county:` chips remain intentionally unsupported (`jobs_detail.json` carries no county FIPS), but no UI path can create one — chips are only ever minted as `state:`/`locality:` (QuickAdd, ScopedAreaActions, Browse add-area button), and `areaCard.ts` + `filters.test.ts` document the skip. Nothing silently empties the Browse list.
  - [x] Routing: `/` → `/browse`; `/browse`; `/map` (preserve all-map); `/localities` (D.5.27 — **shipped 2026-06-26**). *(Root `+page.svelte` redirect changed from `/map` to `/browse`.)*
  - [ ] Masthead pills: `Browse` (default) | `Map only` | `Analysis` (renamed from `Localities` 2026-09-24, ADR-0039). Active-pill state persists in `mapState.viewMode`.
  - [x] `layout.ts` owns the Browse desktop grid. *(Shipped 2026-06-11, adapted to the map-as-home era: instead of per-mode absolute slot rectangles, `layout.ts` exports `BROWSE_MOSAIC` (breakpoint + row/column fractions + named grid areas) and `browseMosaicCss()`, which `/browse` consumes inline — a named-areas CSS grid cannot overlap by construction, so `layout.test.ts` asserts the template's structural integrity (rectangular areas, full-width list row, rev-2 values) instead of pairwise rect checks. Mobile/tablet Browse is the bottom-sheet layout, not slot-positioned panels, so it has no rects to declare.)*
  - [x] Browse layout slots — desktop. *(Shipped 2026-06-11 per the rev-2 mock's proportions, which supersede the 50/50/50 sketch this item originally carried: at ≥ 1024 px — `matchMedia`, so the sheet and the panes are never mounted together — the top band is 30% (map pane 38% wide as context, `BrowseHerePanel` the rest) and `BrowsePostingsPanel` spans the bottom 70% as the work surface. The panes host the *same components* as the mobile sheet: `BrowseSheet` was slimmed to mobile chrome (grabber/detents/swipe pager) around the new shared `BrowseHerePanel` / `BrowsePostingsPanel`. Desktop row click shows the JobCard in the Here pane without moving the map (operator decision 2026-06-11; flyTo + priorViewport stays with the click-to-zoom slice below). Operator decisions 2026-06-11: 720–1023 px gets the full-width map-as-home layout — the 30 rem centered-column clamp was dropped, and a dedicated stacked tablet mosaic is not planned for this slice. WebKit-verified end-to-end in `tests/desktop-mosaic-webkit.spec.mjs`: pane render + pairwise non-overlap, SmallestAreaCard fallback, row-click → JobCard card-only, Edit → FilterSheet, live breakpoint crossing both ways, and post-cycle reactivity. The same session repaired two stale specs: `locality-freeze` (its `.filters-fab` locator became ambiguous when D.6.2 added the "Go to" FAB) and `sheet-detent` (its no-freeze check predated the "+N"-stack → exact-IDs-Postings routing and mis-read that path as a freeze; it now accepts either legitimate outcome).)*
  ...
  - [x] **Sticky list toolbar.** *(Toolbar itself shipped PR #34; the state hoist + round-trip shipped 2026-06-11, WebKit-verified.)* State lives in `mapState.list = { search, sort, facets[] }` (pure model in `jobListFacets.ts`: `ListToolbarState` / `normalizeListToolbar`), encodes to the share URL as `lq` / `lsort` / repeated `lf` (defaults omitted), and rides saved searches **v3** (non-destructive v1/v2 migration). The toolbar now renders on the main Browse Postings pane too — JobList gained a `toolbar` prop so scoped/viewport mode gets search + facets + sort, with facet counts computed off the in-scope base; `/map`'s FeaturePanel keeps the compact sort row (local state). Facet set is the shipped four (GS family / remote-eligible / closing ≤ 7d / hide viewed); a "reposts only" facet can now build on `historical_badge` in a later slice.
  - [x] **Inline list-header annotation (slot).** *(Shipped 2026-06-11.)* The toolbar's summary row renders `mapState.areaPulse?.annotation` as a dashed pill and hides cleanly when null. No stub stat text is rendered while the slice is absent — a fabricated "↑ 23%" would violate the no-invented-stats rule, so absence = hidden, not placeholder copy. Lights up when an `area_pulse` slice populates the store.
  - [x] **Per-job historical badges — live end-to-end.** *(Shipped 2026-06-11, slot + data feed together.)* `src/public_map_export.py::historical_badges` denormalizes the **latest completed** Repost Detector run (`repost_runs` / `repost_groups` / `repost_group_members`) into `jobs_detail.json` as `historical_badge` — "3rd posting · 14 mo", position ranked by `open_date` within the group, month span from the group's earliest member. Rich rows render it as a dashed badge with a "review evidence, not certainty" tooltip (the Repost Detector's own framing). Jobs outside any group carry `null`, never a fabricated badge; databases where the detector never ran export no badges and everything still works. 4 pytest cases cover ordering, span math, latest-run-only, and the null path. Badges appear in the public bundle after the operator's next export following a detector run.
  ...
  - [x] **Pulse band inside `SmallestAreaCard` — LIVE (placeholder 2026-06-11, data 2026-06-12).** Four headline cells — Open postings, New in last 7d, Median window, Closing ≤ 3d — computed client-side by `areaPulse.ts` from the filtered open postings; the new-in-7d delta + annotation come from the trailing-90-day opening-rate baseline (open ∪ closed-overlay postings, ≥ 8 openings required, delta tooltip labels the method as approximate). The card publishes to `mapState.areaPulse` (untracked write; cleared on unmount) and the JobList header annotation reads it. The dashed placeholder rendering remains the no-data state. 10 vitest cases in `areaPulse.test.ts`; WebKit mosaic spec asserts the band goes live with real counts.
  - [ ] **Metrics — folded into the Here card, not a separate tab or pane (rev 2):** rev-1's standalone Metrics dock tab is removed; the metric surfaces render as tap-to-expand content inside `SmallestAreaCard` (the "Here" tab). Four cards: trailing-12-month posting volume (sparkline-style bars), top hiring agencies (horizontal bars), fill signal (median window + reposts + closing-soon counts), workforce-concentration (dashed border, marked `slice-investigate`). Each card carries a `placeholder-tag` until its data source is wired. The "📡 on-demand area enrichment" slot resolved 2026-06-12 per ADR-0036: deterministic + keyless (`AreaWatchNote.svelte`, no external model — see the on-demand area summary checkbox below).
  - [x] `mapState` additions — done as far as the surviving design needs: `list` (search/sort/facets, 2026-06-11), `areaPulse` (nullable, 2026-06-11), `hoveredJobId` (crossfilter, 2026-06-11), plus `listScroll`/`pendingListScroll` (D.5.29's equivalent of `listScrollOffset`). Not added on purpose: `viewMode` (the mosaic↔sheet switch is viewport-derived via `matchMedia`, not stored), `dockTab` (the four-tab dock died with the map-as-home redesign), and `priorViewport`/`selectedJobId` (parked with the click-to-zoom slice; selection rides `selectedFeature`).
  ...
  - [x] Polygon clicks in Browse mode scope the list by geography. *(Shipped #36/#37 as auto-add-on-tap, then **revised 2026-05-27** for the map-as-home redesign: a tap opens the bottom sheet to the area card and an explicit "Add this area to my list" button adds the `state:`/`locality:` chip — operator review rejected auto-adding a chip on every tap. County/marker cards have no add button; the filter model only matches `state:`/`locality:`. `/map` unchanged when `browseMode` is false.)*
  ...
  - [ ] **Data slices to investigate (each tracked separately, none blocking D.5.28 layout):**
    - [x] ~~`area_pulse.json`~~ — **resolved 2026-06-12 as client-side compute, no file and no Function.** A bundle keyed by `(scope_type, scope_code, filter_hash)` is combinatorially impossible (the pulse must honor the live filter set exactly), and lazy-fetch would re-pull what the bundle ships. `areaPulse.ts::computeAreaPulse` counts the four headline numbers from the filtered open postings and derives the new-in-7d delta/annotation from the opening-rate baseline across open ∪ closed-within-90d postings (the exporter now stamps `open_date` onto closed features). Deltas need ≥ 8 baseline openings or they're omitted — no claims on thin data. Powers the Pulse band + list-header annotation today; the fill-signal card can reuse it.
    - [x] ~~`posting_volume_history.json`~~ — **resolved 2026-06-12 as on-demand via the existing `/api/job-history` Function, no new file and no new endpoint.** A precomputed agency × locality × series × month bundle is combinatorially explosive (hundreds of agencies × 58 localities × hundreds of series — far past the 23 MiB guard), and invariant #22 forbids bulk HistoricJoa in the bundle anyway; meanwhile the ADR-0029 Function already returns monthly buckets for agency/series/grade/state slices with a 24 h edge cache. `AreaTrendSparkline.svelte` on the Here card is a click-to-load consumer: pure query mapping + zero-filled months in `areaTrend.ts` (10 vitest cases), fixed `window=1yr`, locality scope falls back to the locality's primary state with an explicit "approximate" note (HistoricJoa has no locality filter), extra agency/series chips beyond the first are named in a note rather than silently dropped, filter drift after load shows a reload notice instead of auto-refetching, and upstream failure renders "Trend unavailable" — never fabricated bars. WebKit-verified end-to-end in `tests/area-trend-webkit.spec.mjs` (click-to-load gate, 13 zero-filled bars, stale→reload, failure mode).
    - [x] Per-job historical badge denormalization — **done 2026-06-11** (see the per-job historical badges checkbox above; `historical_badges()` in the exporter).
    - [x] ~~On-demand area summary (`functions/api/area-summary.ts`)~~ — **resolved 2026-06-12 as deterministic + keyless, per ADR-0036 (operator decision): no LLM, no new Function, no secret in the deployment.** `AreaWatchNote.svelte` on the Here card is a click-to-load consumer of the existing `/api/job-history` Function at `window=3yr`; `areaWatch.ts::computeWatchNote` derives year-over-year, seasonal-peak, and median-posting-window claims client-side, each behind an explicit evidence bar — claims that fail the bar are rendered as "withheld" with the reason (truncated payload, < 24 complete months, thin prior year/sample), never silently dropped or fabricated. 15 vitest cases in `areaWatch.test.ts`; WebKit-verified in `tests/area-watch-webkit.spec.mjs` (click-to-load gate, claim rendering, withheld reason, stale→reload, failure mode). The rev-1 mock's external-model sketch is superseded; revisiting an LLM variant requires a new ADR per ADR-0036.
...
- [x] **D.5.29 — Shareable view URLs (per ADR-0033).** *(Shipped 2026-06-11, PR #71; WebKit-verified. **KV binding done + short links verified live in production 2026-06-11** — `POST /api/share` → 200, `GET /s/<hash>` → 302 `/browse?…`, bad hash → expired page. No outstanding operator setup.)*
  - [x] URL state extended to the full view: filters + metric + viewport (`center`,`zoom`) + theme + `selected=<jobId>` + `scroll=<0..1>` + radius chips. Pure codec in `public_map/src/lib/viewState.ts` (`writeViewToParams`/`readViewFromParams`/`viewToParamString`/`shareUrlFromView`), delegating the filter keys to `filters.ts`. `viewSync.ts` bridges the codec ↔ store (`readCurrentView`/`applySharedView`). `/browse` hydrates the whole view on mount and keeps the address bar in sync (debounced `replaceState`).
  - [x] `Share` button (`ShareLinkButton.svelte`) in the masthead — on `/browse` (next to Saved) and `/map` (next to Pay Compare).
  - [x] `public_map/functions/api/share.ts`: POST stashes the param string in KV namespace `fedfinder_share` under a deterministic 7-char base32 hash of it (idempotent — identical views reuse the key), `expirationTtl: 7776000` (90 days); returns `{hash, path, url}`. 503 when no KV binding so the client falls back. Pure hashing in `src/lib/share.ts`.
  - [x] `public_map/functions/s/[hash].ts`: GET reads KV, 302-redirects to `/browse?<params>`; KV miss / bad hash / no binding renders a friendly "this share link has expired" page (200, links to `/browse`) — never a 404. *(Path is `/s/<hash>` — the app serves at the subdomain root. CLAUDE.md invariant #28's `/map/s/<hash>` is reconciled to `/s/<hash>`.)*
  - [x] KV namespace setup documented in `docs/REMOTE_OPERATIONS.md` (create `fedfinder_share`, bind to variable `fedfinder_share` on Prod + Preview).
  - [x] Failure mode: non-2xx from the Function → the long URL is copied with a "short link unavailable" toast. Clipboard copy always succeeds (async clipboard + execCommand fallback).
  - [x] Closed-job fallback: a shared `selected=<jobId>` that is no longer open renders the rest of the view plus a dismissable `ShareClosedBanner` ("the highlighted posting has closed…"). Resolution is deferred until `jobs_detail` loads; the effect wraps its `mapState` writes in `untrack`.
  - [x] Saved-searches schema v2 (radius chips) — already shipped in D.5.30; unchanged here.
  - [x] Tests: `viewState.test.ts` (full round-trip incl. selected/scroll/radius, default omission, malformed-param fallbacks, idempotent rewrite), `share.test.ts` (base32 + deterministic hash + validators), `shareFunction.test.ts` (POST mints/idempotent/503/400, GET 302/expired-page/no-binding with a mocked KV), and `tests/share-urls-webkit.spec.mjs` (real WebKit: hydration, short-link success, long-URL fallback, closed-job banner, live-selected card). 226 vitest pass.
- [x] **D.5.30 — Radius search (per ADR-0033).** *(Shipped 2026-06-10; WebKit-verified.)*
  - [x] `AddressSearch.svelte` commits a chip on selection (the new `commitRadius` prop, on for the Browse "Go to" panel). Chip: `{ center: [lng, lat], miles: 25 | 50 | 100 | 250, label, includeRemote }`, default 50 mi.
  - [x] Chip UI in `ActiveFilterStrip`: "Within 50 mi ▾ of Chicago · +remote · ×"; the `▾` opens a four-option popover, the `+remote`/`no remote` button toggles anywhere-remote inclusion, and the `×` removes the chip. (Browse uses the docked `ActiveFilterStrip`, not `FilterPanel`, post-D.6.)
  - [x] Filtering: new pure module `public_map/src/lib/geo.ts` (`haversineMiles`, `radiusMatch`, `coordsByJobId`, URL helpers). Both `matchesJobFeature` (map markers, per-feature geometry) and `matchesJobDetail` (rich list, via a posting-id→coords index built from jobs.geojson) honor radii; multi-location matches if any duty station is in range; anywhere-remote included by default with a per-chip exclude toggle.
  - [x] URL state: repeated `radius=lng,lat,miles[,xr]` keys (`xr` = exclude remote). Per invariant #17, radius chips OR with geography chips and with each other, and the whole location group ANDs with non-geographic filters.
  - [x] `JobList` rows (both rich and scoped/viewport modes) show distance from center when exactly one radius chip is active; a "Nearest" sort option appears when ≥1 radius chip is set.
  - [x] List header counter: "N within 50 mi of Chicago + M remote" (the remote tail shows only when remote inclusion is on and some matched).
  - [x] Saved searches bumped to schema **v2** with non-destructive v1 migration (v1 stores load with empty radii).
  - [x] Tests: `geo.test.ts` (haversine vs. NY/LA/Chicago/Anchorage, radiusMatch in/out + remote toggle + multi-location + OR, coordsByJobId, URL round-trip, normalize/dedup) and radius cases in `filters.test.ts` (in/out, remote default+exclude, OR-with-geography, multi-location, URL round-trip). 189 vitest pass. `tests/radius-search-webkit.spec.mjs` drives the full flow on real WebKit (geocode→chip→popover→remote toggle→summary→remove→no freeze). **Deferred to D.5.29:** persisting the in-list toolbar's distance sort to the URL, and `radius=` short-link sharing.
...
### Phase D.6 — Mobile cohesion (ADR-0035)
