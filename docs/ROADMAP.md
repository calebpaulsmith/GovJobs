...
- [~] **D.5.28 — Browse-first mosaic redesign (per ADR-0033).** *(Mocks shipped 2026-05-11, then re-cut to **rev 2** on 2026-05-12 — see `public_map/mocks/browse/`. Rev 2 is the current target: one filter bar (no separate metric switcher), metrics folded into the area card, "Save as Job List" / "Save to My Postings" verbs with a provenance toast. The mobile dock is **four tabs — Map / List / Here / Saved**; rev-1's separate "Metrics" tab is gone and "Area" is renamed "Here".)*
  - **Status (2026-05-25).** Shipped: the `/browse` route + mobile dock shell + List tab (PRs #22/#24), `locality_code` export + locality-filter fix (#28), the JobList unification (#29), the Here tab `SmallestAreaCard` (#32), the Saved tab (#33), the JobList rich-mode sticky toolbar + facets (#34), the live Map embedded in the Map tab (#35), Browse-mode polygon-click → `geo:` chip (#36/#37), and summary/qualifications excerpts in rich rows (≤ 200 chars, HTML-stripped at export in `src/public_map_export.py::_excerpt` → `jobs_detail.json`; 6,419 populated in the latest bundle; landed via a direct-to-master commit, not a numbered PR). **All four dock tabs (Map / List / Here / Saved) are now functional.** Still open below: the desktop mosaic (exists only as the `desktop-mosaic.html` mock — desktop ≥ 720 px today is just the mobile dock centered in a 30 rem column), marker-click → JobCard overlay + row↔marker crossfilter, and the `AgencyPicker` ↔ `FilterPanel` reconciliation. D.5.28 stays `[~]` until those land.
  ...
  - [ ] `layout.ts` accepts `mode: 'browse' | 'map-only'`; per-mode slot rectangles for desktop (≥ 1024 px), tablet (640 – 1023 px), mobile (< 640 px). **Browse-mode mobile (< 640 px) layout is dock-driven, not stacked:** a fixed bottom dock with **four tabs (Map / List / Here / Saved)** swaps the content area; only one pane is visible at a time. Tests assert non-overlap for both modes at all three breakpoints + dock-tab transitions don't overlap masthead or scope bar.
