...
- [~] **D.5.27 — Localities screen (per ADR-0032).**
  - **Status (2026-06-26).** V1 SHIPPED: `/localities` route + enabled masthead pill on `/browse`; `LocalityRollup.svelte` centerpiece (sortable Locality / Postings (default desc) / Pay range / Pay-plan mix + GS-coverage / Cost-of-living-RPP columns, each with a D.5.13 InfoTooltip); multi-select + "Show N jobs in M localities" drill-in and single-row shortcut, both encoding `locality:<code>` chips into the `/browse` URL (preserves non-geographic filters, survives Browse's URL hydration, and is shareable); one-click Remote-only preset; JobList anywhere-remote redesignation note (ADR §9). Rollup math lives in `public_map/src/lib/localities.ts` and aggregates over **jobs.geojson features** (which carry `locality_code`; `jobs_detail.json` does not), deduping postings per locality and dropping geography/radius chips. `manifest.localities_screen_v1: true`. Tests: `localities.test.ts` (11) + `tests/localities-webkit.spec.mjs` (11, real WebKit). **Deferred to fast-follows** (both optional/supporting per ADR-0032 §2/§5, not in the exit criteria): the opt-in GS-anchored purchasing-power column and the paired highlight map. FRPP-count column dropped from V1 — `federal_properties` has no locality join (point-in-polygon needed); revisit with V1.1.
