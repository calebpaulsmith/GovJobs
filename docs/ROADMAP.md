...
- [~] **D.5.28 — Browse-first mosaic redesign (per ADR-0033).** *(Mocks shipped 2026-05-11, then re-cut to **rev 2** on 2026-05-12 — see `public_map/mocks/browse/`. Rev 2 is the current target: one filter bar (no separate metric switcher), metrics folded into the area card, "Save as Job List" / "Save to My Postings" verbs with a provenance toast. The mobile dock is **four tabs — Map / List / Here / Saved**; rev-1's separate "Metrics" tab is gone and "Area" is renamed "Here".)*
  ...
  - **Cleanup — mostly resolved.** The List tab no longer carries a parallel inline list: `JobList.svelte` is the single list component, used in `richMode` by `/browse` and scoped mode by `/map` (shipped #29). **Still pending:** reconcile `AgencyPicker.svelte` (used by /browse) with `FilterPanel`'s inline agency picker (used by /map) so there is one implementation.
  - **Known bug.** `matchesJobDetail` / `jobDetailGeographyMatches` in `filters.ts` cannot match `locality:` geography chips (`jobs_detail.json` carries no `locality_code`) and ignores `county:` chips entirely — such a chip silently empties the Browse list. Fix with the JobList rework: add `locality_code` to the `jobs_detail.json` export, or route Browse through the map's existing `filterJobs` path.
