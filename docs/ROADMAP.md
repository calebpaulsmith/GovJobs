...
- [ ] D.5.2 — Agency multi-select with code-backed typeahead and aliases (URL state via repeated `agency=` keys). *(Initiated 2026-05-09 as the next public-map step. Build order: add/export agency option metadata, replace the free-text control with a multi-select typeahead, serialize selected codes as repeated `agency=` URL keys, and verify marker/heat/list filtering against the same code set.)*
