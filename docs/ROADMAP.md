...
- [~] **D.5.28 — Browse-first mosaic redesign (per ADR-0033).** *(Mocks shipped 2026-05-11, then re-cut to **rev 2** on 2026-05-12 — see `public_map/mocks/browse/`. Rev 2 is the current target: one filter bar (no separate metric switcher), metrics folded into the area card, "Save as Job List" / "Save to My Postings" verbs with a provenance toast. The mobile dock is **four tabs — Map / List / Here / Saved**; rev-1's separate "Metrics" tab is gone and "Area" is renamed "Here".)*
  - **Status (2026-05-19).** Shipped: the `/browse` route + mobile dock shell + List tab (PRs #22/#24), `locality_code` export + locality-filter fix (#28), the JobList unification (#29), the Here tab `SmallestAreaCard` (#32), the Saved tab (#33), the JobList rich-mode sticky toolbar + facets (#34), the live Map embedded in the Map tab (#35), and Browse-mode polygon-click → `geo:` chip (this PR). **All four dock tabs (Map / List / Here / Saved) are now functional.** Still open below: the desktop mosaic (exists only as the `desktop-mosaic.html` mock), summary/qualifications excerpts in rich rows, marker-click → JobCard overlay + row↔marker crossfilter, and the `AgencyPicker` ↔ `FilterPanel` reconciliation. D.5.28 stays `[~]` until those land.
  ...
  - [x] Polygon clicks in Browse mode additively add a `geo:` chip and narrow the list, on top of the existing fit-bounds + `ScopedAreaActions` behavior from D.5.15. *(Shipped: `Map.svelte` gains a `browseMode` prop; on `/browse` a state-fill click adds `state:<code>` and a locality-fill click adds `locality:<code>` to `mapState.filters.geographies`. County/metro clicks add no chip — the filter model doesn't match those. `/map` behavior is unchanged when `browseMode` is false.)*
