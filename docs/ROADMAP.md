...
- [ ] D.5.5 — Click-state-to-fit-bounds (also locality / county / CBSA at their zoom bands). *(Initiated 2026-05-09: polygon clicks in `Map.svelte` now compute feature bounds, fit to state/locality/county/metro zoom bands, and show a Back to national pill. Remaining: visual verification and scoped action window follow-through.)*
...
- [x] D.5.10 — County-level COL via Census ACS rent joined into `cost_of_living_index`. *(Shipped 2026-05-09 per ADR-0031: new `census_acs_rent` source key. `scripts/ingest_acs_county_rent.py` reads `data/external/census_acs_rent/2023.csv` (or operator-supplied `--input` / `PUBLIC_MAP_ACS_COUNTY_RENTS_CSV`), computes per-state median rent from the input, looks up the latest BEA state RPP, and writes derived `(year, geo_type='county', geo_code=fips, rpp_overall, rpp_rents=median_rent_$, source='census:acs5_b25064')` rows. Orchestrator runs the ACS step right after `ingest_bea_rpp` so the state RPP is in place. Seed covers 50 states + DC + 100+ counties so a clean checkout produces meaningful within-state variance immediately. New `_county_col_lookup` + extended `counties_geojson` surface `rpp_overall_source` (`'county'` | `'state'`), `rpp_state` (always state RPP for tooltip math), `rent_median`, and now compute `gs13_step1_locality` + `pay_vs_col` per county. `cost_of_living()` payload gains `by_county` keyed by 5-digit FIPS. `CountyDetail.svelte` adds a Median gross rent row, swaps the RPP heading to "County COL index" when ACS data is present, and updates the InfoTooltip with the formula + source labels. Optional BLS metro CPI is deferred — not blocking V1 per the ADR. Tests in `tests/test_ingest_scripts.py` cover state-RPP-aware and fallback paths plus the seed loadability; `tests/test_public_map_export_polygons.py` covers the county feature and the `by_county` payload.)*
...
- [ ] **D.5.24 — On-demand Posting Intelligence (per ADR-0029).** Click-to-load tab on JobCard powered by a Cloudflare Pages Function (`public_map/functions/api/job-history.ts`) that proxies USAJOBS HistoricJoa with a 24h edge cache. Two surfaces: (a) a filtered timeline chart of monthly posting counts matching the user's active filters with time-window pills (1mo / 3mo / 6mo / 1yr / 3yr / 5yr / 10yr); (b) a "See all N matching historic postings" drill-in list, paginated, also Function-backed. No bulk historic import. No prefetch — explicit user click only. Inherits `mapState.filters` as single source of truth (no parallel filter UI). Failure mode: explicit "history unavailable" message, never fake data.
- [ ] **D.5.25 — Geographic detail card hierarchy parity.** Close the gap between the four geography surfaces (national / state / locality / county) so each one is a first-class detail card with the same scoped-action contract. Two pieces:
  - **`NationalRoundup.svelte`** — default `FeaturePanel` content when no feature is selected. Aggregates US-level signal: total open postings, total federal workforce / accessions / separations from `opm_state_aggregates`, median GS-13 step 1 across localities, median state RPP, top 5 agencies by openings (from `agencies.json`), top 5 states by openings, and a national pay-vs-COL summary. The "Back to national" pill opens this panel as well as clearing focus. Layout cell already exists (`feature-panel`); declare a `national` panel kind in the FeaturePanel switch.
  - **`ScopedAreaActions.svelte` parity for `CountyDetail`.** D.5.15 shipped scoped actions on State and Locality. Add the same component to `CountyDetail.svelte` with `scope = { kind: 'county', code: <fips>, label: <county name> }`. Polygon-click handler in `Map.svelte` already fits county bounds (D.5.5); this slice opens the action window in the panel and routes the chip through the same path State and Locality use.
- [ ] **D.5.26 — County as a first-class geography filter chip.** Lifts D.5.15's `geographies: string[]` chip model from `state:<code>` / `locality:<code>` keys to also support `county:<5-fips>`. Required pieces:
  - `_feature_from_marker` (in `src/public_map_export.py`) currently keeps `county_fips` in the marker dataset but does not emit it on `jobs.geojson` properties. Add `county_fips` to the per-feature properties so client-side filtering can match it. `jobs_detail.json` should also expose `county_fips` for any consumer that needs the full list of locations.
  - `filterJobs` (in `public_map/src/lib/filters.ts`) must accept `county:<fips>` chips and OR them across with state/locality chips, AND with everything else. URL state continues to use repeated `geo=` keys.
  - `ScopedAreaActions` in CountyDetail (D.5.25) writes `county:<fips>` chips through the same `mapState.filters` path. `QuickAdd.svelte` learns a `geography` chip wired to the county FIPS shown on JobCard's "Clicked location" row and CountyDetail.
  - `FilterPanel` chip rendering must label county chips with the county name (e.g. "Cook County, IL") not just the FIPS — pull labels from `counties.geojson` via a small lookup helper.
