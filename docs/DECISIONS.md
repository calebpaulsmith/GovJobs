...
5. **Polygon-click implicit chip behavior in Browse mode.** ~~Proposed:~~ **Resolved (2026-05-24, D.5.28):** yes. `Map.svelte` takes a `browseMode` prop set only on `/browse`; a state-fill click adds `state:<code>` and a locality-fill click adds `locality:<code>` to `mapState.filters.geographies` (deduped, uppercased to match `ScopedAreaActions`), additively narrowing the list on top of the existing fit-bounds + ScopedAreaActions flow. County/metro clicks add no chip — `jobs_detail.json` carries no county FIPS and the filter model only matches `state:`/`locality:`. `/map` behavior is unchanged when `browseMode` is false.
