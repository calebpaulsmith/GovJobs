...
**Verification log (2026-06-10) — Stage 2 complete; Stage 1 assumption was wrong.** With OPM now reachable from the sandbox, the official 2026 GS and DCB tables were fetched from opm.gov and compared to the seed the ingest actually reads. Two findings: (1) The real seed is **`data/external/opm_gs_pay/2026.official.csv`** (8,850 rows = 150 base + 58 locality areas × 150), sourced from OPM's `salary-tables/xml/2026/GS.xml` — it had already replaced the bootstrap before this verification, and it matched opm.gov on **all 150 base cells** plus a DCB locality spot-check (GS-13 step 1 = $121,785 at the official 33.94% adjustment). D.5.14 is verified and ROADMAP flips to `[x]`. (2) **Stage 1's `round(2025 × 1.01)` premise was incorrect.** OPM restructured the 2026 base/locality split, so the official 2026 *base* is **lower** than 2025 (GS-1/1 = 22,584 vs 22,886) — the across-the-board 1% applies to total comp, not the notional base table. The bootstrap `2026_base.csv` (which used the `× 1.01` rule) was off by hundreds-to-thousands of dollars per cell, was never read by any code (it was orphaned when `SEED_CSV` was repointed to `2026.official.csv`), and has been deleted. Lesson for future year cutovers: **fetch OPM's published table; never derive the base from the prior year by the headline raise %.** `tests/test_ingest_scripts.py::test_gs_pay_seed_is_official_opm_2026_not_placeholder` pins the official checkpoints; the admin panel now auto-compares `pay_scales` against them.
...
4. **Shareable view URLs.**
   ...
   - `Copy share link` button in the masthead next to the Saved Searches menu. Clicking it POSTs the full param string to a new Cloudflare Pages Function `public_map/functions/api/share.ts`, which writes to KV namespace `fedfinder_share` keyed by a 7-character base32 hash and returns the short URL `https://map.thegrandpipeline.com/s/<hash>`. TTL is 90 days via `expirationTtl: 7776000`. (The app serves at the `map.` subdomain root, so the short link is `/s/<hash>`, not `/map/s/<hash>`.)
   - Short-link resolution: `public_map/functions/s/[hash].ts` reads KV and 302-redirects to `/browse?<params>`. A KV miss renders a friendly "this share link has expired" page with the current `/browse` (unfiltered). (CLAUDE.md invariant #28 still names `functions/map/s/[hash].ts`; drop the `/map/` segment when this feature is built.)
...
5. **Polygon-click implicit chip behavior in Browse mode.** ~~Proposed:~~ **Resolved, then revised (2026-05-27, D.5.28 map-as-home):** a polygon tap does **not** auto-add a chip. The first cut (PRs #36/#37) auto-added a `state:`/`locality:` chip on every tap; operator review found that "a million baby chips" accumulate as you scroll and tap around. New behavior: in `browseMode`, a tap on a state / locality / county / marker sets `mapState.selectedFeature` and opens the bottom sheet to that area's card; adding the area to the working list is an **explicit "Add this area to my list" button** on the card (deduped, uppercased `state:`/`locality:` chip — same format as `ScopedAreaActions`). County/marker cards have no add button — the filter model only matches `state:`/`locality:`. `/map` behavior is unchanged when `browseMode` is false (localities there still open a scoped job list).
...
8. **Bundle-size ceiling.** ~~If `summary_excerpt` + `qualifications_excerpt` push the bundle over 25 MiB, switch to lazy-load excerpts via a Cloudflare Pages Function fed by the local DB snapshot.~~ **Resolved (2026-05-25, D.5.28):** excerpts shipped inline in `jobs_detail.json` and the existing split-bundle path (`scripts/bundle_split.py`, 20 MiB threshold) keeps each emitted file under Cloudflare's 25 MiB per-file limit (`jobs_detail.json` ≈ 11.7 MiB + `jobs_detail.2.json` ≈ 13.8 MiB in the latest bundle). The lazy-load Pages Function was not needed. If a future bundle pushes a single split part over 25 MiB despite splitting, revisit lazy-load and document the threshold in `docs/PUBLIC_MAP_PIPELINE.md`.
...
## ADR-0037 — Overseas US-federal postings: international work locations, not foreign governments
Date: 2026-06-20
Status: Accepted

**Context.** USAJOBS posts many **US federal** jobs located abroad — State Dept embassies, DoD bases, USAID missions (Rome, Tokyo, Mexico City, etc.). The dashboard silently assumed every posting is domestic: the `jobs` table denormalized only `state`/`city` (no `country`), the Search UI filtered on a 2-char US `state`, `pay_calculator` fell back to "Rest of US" (0% adjustment) for any location it could not map to a US county, and `scoring._score_location` penalized non-Midwest states by −3. Overseas postings already import cleanly (`usajobs_normalize` captures `CountryCode`/`positionLocationCountry` into `job_locations.country`; no import filter drops them), but they were invisible to filters, silently mispriced, and wrongly penalized.

An earlier AI-generated proposal misread the goal as ingesting *other countries' governments* via foreign job boards (EURES, France Travail, etc.). That is explicitly rejected: candidates for foreign civil services are a different audience and a different product. The goal is narrower and coherent — **the same US-federal-jobs experience, with the location not narrowed to the US.**

**Decision.**
1. **Scope: US federal only; location is the only thing that internationalizes.** No foreign job sources, no foreign grade systems. Postings stay USAJOBS-sourced; pay stays **GS / USD**. Federal *contracting* jobs are a separate, later effort.
2. **One schema change: denormalize `country` onto `jobs`** (TEXT, indexed `idx_jobs_country`), mirroring how `state`/`city` are already denormalized, because the UI filters/groups on it (per `SCHEMA_EXPANSION_PLAN.md`). Existing rows backfill from the primary `job_locations.country`, defaulting to `'US'`. Schema version → 12.
3. **Reject currency/grade-system/occupation/sector columns.** They came from the foreign-government misread; overseas US-fed pay is still GS in USD, so they add schema with no consumer.
4. **Canonicalize country across sources.** Search supplies ISO alpha-2 codes (`IT`), HistoricJoa supplies names (`Italy`); `database.normalize_country()` collapses both to one form so filters don't fragment. `database.is_overseas()` is the single shared definition of "overseas," and **US territories (PR, GU, AS, MP, VI) are domestic** — they keep US locality pay and US-geography scoring.
5. **Honesty before silence in pay/scoring.** Overseas postings must never be silently priced as Rest-of-US 0% nor penalized as "outside Midwest." Pay shows GS base with an explicit "overseas allowances apply" label until DSSR data lands; scoring gives overseas a neutral location factor (no bonus, no penalty).
6. **Real overseas pay via State Dept DSSR** (post/COLA allowance, hardship differential, danger pay) is a dedicated later stage with its own table and self-bootstrapping ingest (per ADR-0027) — **not** a widening of `pay_scales` (ADR-0034 no-widen rule binds). Operator chose to build this for real, not just label its absence.
7. **Reuse the existing world map.** `pages/4_State_Map.py` already uses worldwide Folium tiles and plots any row with coordinates; overseas support is geocoding the gap and relaxing US-only filters, not a new map. The Svelte `public_map/` product stays US-scoped and must count-and-label (never silently drop, never leak with fake coords) overseas rows.

**Consequences.** A small, reversible schema seam unlocks overseas search/filtering immediately; the heavier pay (DSSR) and map work are gated stages behind it. The `normalize_country`/`is_overseas` helpers in `src/database.py` are the shared contract reused by `usajobs_normalize`, `pay_calculator`, and `scoring`. The work is staged with gates (Stage 0 spike → schema → surface → honesty fixes → DSSR pay → map); see the plan of record. No change to the local-first / no-cloud posture.

**Stage 4 addendum (2026-06-20) — real DSSR overseas pay.** The "later stage" in point 6 shipped. Decisions, grounded in a live source review:

- **Source: live HTML from `allowances.state.gov`** (aoprals.state.gov 302-redirects there; verified 2026-06-20). The site publishes **HTML tables only** — no CSV/XML/bulk export — so `scripts/ingest_dssr_allowances.py` parses the three Web920 tables (`hardship.asp`, `danger_pay_all.asp`, `cola.asp`) and the Spendable Income `.docx`. Operator chose live fetch as the canonical refresh.
- **Ordering: live-first, seed-floor** (a deliberate departure from `resolve_or_download`'s seed-before-network order). Live fetch is the canonical refresh; on any failure (offline, format drift, ToS block) the ingest falls back to checked-in seed CSVs captured from the live tables (`data/external/dssr/post_allowances_2026-06-14.csv` = 792 posts; `spendable_income_2023-01-01.csv` = 22 bands × 6 family sizes). A clean/offline checkout still builds (satisfies ADR-0027 / hard rule 14). This is dashboard pay data, **not** public-map data, so it is intentionally **not** wired into `scripts/refresh_public_map_data.py`.
- **Two new tables (schema v13), not a `pay_scales` widening** (ADR-0034 no-widen rule binds): `overseas_post_allowances` (country_iso/name, post, the three percentages, effective date, source) and `spendable_income` (salary band × family size → annual spendable income).
- **The comp model has three distinct shapes, and COLA is the trap.** Hardship (DSSR 500) and danger (DSSR 650) are exact **% of base pay**. Post (COLA) allowance (DSSR 220) is **% of *spendable income*, not base** — its dollar value needs the Spendable Income Table and depends on family size, so it is presented as a **flagged estimate** (single-filer default) and the dollar figure is **withheld with a reason** (never guessed) when salary is below the published table.
- **Post matching** reuses `normalize_country` (ISO) and State's own per-country `"Other"` row as the country-level fallback: exact (iso, city) → (iso, "Other"), flagged approximate → unmatched (GS base only, explicit note). Reads live in `src.reference_data.overseas_compensation` (the traceable line-item breakdown surfaced in the Search deep-research expander) and `overseas_post_allowance`; `pay_calculator` attaches the matched percentages as an `overseas_allowances` block. US output stays byte-for-byte unchanged.

## ADR-0038 — Overseas US-federal postings are first-class on the public website (supersedes ADR-0037 §7)
Date: 2026-06-23
Status: Accepted
Supersedes: ADR-0037 §7 ("the Svelte `public_map/` product stays US-scoped") and its Deferred item "Overseas support inside the Svelte `public_map/` product."

**Context.** ADR-0037 shipped overseas US-fed support (country denormalization, honest GS-base + DSSR pay, world-map markers, geography-neutral scoring) **into the Streamlit dashboard only**, and explicitly kept the public Svelte site (`map.thegrandpipeline.com`, `/browse` + `/map`) US-scoped. On 2026-06-20 the operator corrected the target: *"No time on the Streamlit… the product is the website (phone + desktop). The overseas work needs to go into the website."* Under that goal ADR-0037 §7 was the wrong call — the overseas work was invisible to the people who actually use the product. The fix is to port the dashboard's overseas capability onto the website by **reuse, not reinvention** (per ADR-0035): the website renders what the one-way exporter precomputes; it does not recompute pay or re-derive geography.

**Decision.** Overseas US-fed postings are first-class on the public website, shipped as four small, separately-tested, WebKit-gated increments (PRs #78, #79):

1. **Bundle carries overseas (5′.1).** `src/public_map_export.py` stops excluding overseas rows: markers carry `country` and a `geo_quality` of `country_centroid` (last-resort coordinate from the schema-v14 `country_centroids` seed) when USAJOBS gave no duty-station coords, else `exact`. `jobs_detail.json` carries `country` (US default) and an `overseas_pay` block reused verbatim from `src.reference_data.overseas_compensation` (the same traceable DSSR breakdown the dashboard shows — **one source of truth, no second pay path in TS**). A `countries.json` option catalog (names joined from `country_centroids`) feeds the filter. **US output is unchanged.**
2. **Map shows overseas, labeled approximate (5′.2).** Country-centroid markers render **amber + translucent** (distinct from precise blue) with an "approximate — country centroid, not the duty station" popup line. US choropleth/polygons stay US-only (they simply don't cover overseas); the camera may leave US bounds.
3. **JobCard renders the DSSR breakdown (5′.3).** When `overseas_pay` is present, the card shows GS base + hardship/danger (exact % of base) + COLA (flagged **est.** or **withheld** with reason) + post-match precision + per-line source links, and suppresses the US-centric "pay scale not ingested — see admin" message (misleading overseas). The TS formatter (`overseasPay.ts`) only formats — it does no pay math.
4. **Country scope filter (5′.4).** A `countries` multi-select mirrors the agency filter (code-backed chip, repeated-key `country=` URL codec, saved-search schema **v4**). **Country is a *categorical* filter, matched independently — deliberately NOT part of the geographies/radii OR-group** (invariant #17), so it still narrows **ungeocoded** overseas postings (which have no coordinates) via `ungeocodedFilteredDetails`. The control hides when the bundle offers ≤ 1 country, so US-only deployments are visually unchanged.

**Consequences.** The dashboard remains the analyst surface but is no longer the *only* place overseas data lives. The honesty contract is preserved end-to-end: approximate coordinates and estimated/withheld pay are labeled on the website exactly as in the dashboard; no overseas number is recomputed client-side. Bulk historic imports are still rejected (ADR-0029 / invariant #22). Remaining overseas work (street-level overseas geocoding precision; federal *contracting* jobs) stays deferred. The `public_map/` "no overseas" stance in ADR-0017/0037 is closed.

## ADR-0039 — Browse finds jobs; Analysis analyzes places (the Here tab moves to a renamed Localities screen)
Date: 2026-09-24
Status: Accepted
Supersedes: ADR-0033 decision #2's "Here" pane (SmallestAreaCard beside the list) and the Browse sheet's Here/Postings pager; renames ADR-0032's Localities screen. Amends invariants #26, #27, #29.

**Context.** The operator reviewed mobile Browse and found the "Here" tab was analysis, not job-finding: it auto-narrowed to the "smallest area containing the active filter" and showed the pulse band, pay vs. COL, workforce, urgency, the 12-month trend, and "What to watch". Mixing that into the find-jobs surface blurred the product. Operator direction (2026-09-24): *"In one tab we look for jobs, in the other we analyze trends and metrics."* The Here content should move under the Localities pill, renamed **Analysis**, and instead of auto-narrowing it should let the user **pick the level** — defaulting to the level that matches where they were.

**Decision.**
1. **Two jobs, two surfaces.** `Browse` (find and apply) | `Map only` (desktop analyst map) | `Analysis` (trends and metrics). The Localities route is renamed `/analysis`; `/localities` redirects there, carrying its query string.
2. **Browse holds only job things.** The mobile bottom sheet is Postings-only (no Here tab, no swipe pager). Picking a job — map marker, same-point stack, or list row — opens its card as a detail view over the list with "‹ Back to postings"; the list keeps its scope (JobList `keepScopeOnPick`). The desktop mosaic's top-right pane shows the selected job card (or a "Job details" prompt that points area questions to Analysis). Tapping a state / locality / county / metro narrows the Postings list to it and offers **"Analyze this area →"**; the "Add to my list" chip stays explicit (ADR-0033 #5). The first-run welcome card moves to the top of the Postings list. A shared `selected=` job now opens a real JobCard.
3. **Analysis analyzes one chosen area at one of five levels:** Nationwide · State · Locality · Metro (CBSA) · County (operator chose to include Metro and County). `AreaAnalysis.svelte` (the former SmallestAreaCard) takes the area explicitly; `analysisArea.ts` owns the model.
4. **Default area = where the user was in Browse:** the polygon they tapped > their geography chip (locality > state) > the Browse map center at a level chosen from its zoom (< 5 nationwide, < 6.5 state, < 8 locality, else county), falling back county/locality → metro → state → nationwide when nothing is under the point. An explicit `?area=level:code` wins. Switching level keeps the user in place via the area's anchor point; the typeahead picks any area by name.
5. **Scoping is honest.** Analysis applies the user's *non-geographic* filters (shared with Browse) and replaces Browse's geography/radius chips with the chosen area (a note says so). Jobs carry no county FIPS or CBSA code, so county/metro scoping is **client-side point-in-polygon over duty-station coordinates** (`geo.ts::pointInGeometry`); postings with no mappable duty station are named as excluded, never silently dropped. The same point-in-polygon fixes Browse's county/CBSA list scopes, which previously matched nothing. Workforce stays state-only; metros show RPP but no pay-vs-COL index ("not computed for metros"); the HistoricJoa trend/watch use the area's primary state for locality/metro/county with an explicit "approximate" note (ADR-0029 has no finer filter).
6. **The locality comparison (ADR-0032 rollup + mini map) stays**, as a "Compare locality pay areas" section below the area analysis.

**Consequences.** Browse is simpler and single-purpose; analysis is richer (any level, any area) and URL-shareable (`/analysis?area=…&<filters>`). `mapState.areaPulse` is no longer published (the Analysis card no longer feeds the Browse list header annotation — area context belongs on Analysis); the JobList annotation slot stays inert. Tab switches preserve filters: Analysis's Browse link carries the full view (`writeViewToParams(readCurrentView())`). `SmallestAreaCard`/`BrowseHerePanel` are retired (`AreaAnalysis`, `BrowseJobPanel`, `BrowseWelcome` replace them). WebKit specs updated; new `tests/analysis-webkit.spec.mjs` covers both surfaces.
