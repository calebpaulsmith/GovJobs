...
- Phase F (initiated): the public map now includes `/about` attribution/disclaimer content, footer navigation to that page, Open Graph/Twitter share metadata, `public_map/static/robots.txt`, `public_map/static/sitemap.xml`, and `public_map/static/og-image.svg`. Keep these public-surface assets aligned with `docs/PUBLIC_MAP_PIPELINE.md` and `docs/ROADMAP.md` when adding analytics or deployment polish.
